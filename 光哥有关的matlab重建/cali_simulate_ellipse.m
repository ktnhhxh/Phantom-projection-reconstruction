...
D_Vec_Matrix_ellipse=D_Vec_Matrix;
...
SourcePos_Psi_ellipse = SourcePos_Psi;
for index=1:360
...
for i=1:Proj_Hei       %%%%真实探测器上的y坐标
        ...
        for j=1:Proj_Wid   %%%%真实探测器上的x坐标
            ...
            SourcePos_Psi_ellipse(1)=2*sqrt(power(aa*cospsi,2)+ power(bb*sinpsi,2));
%             DirInOmigaAlphaBeta_ellipse=P_3D_Pt_Psi-SourcePos_Psi_ellipse;
%             DirNorm_ellipse=norm(DirInOmigaAlphaBeta_ellipse);
%             NormVec_ellipse=DirInOmigaAlphaBeta_ellipse/DirNorm_ellipse;
            
            DirInOmigaAlphaBeta = P_3D_Pt_Psi - SourcePos_Psi_ellipse;
            DirNorm  = norm(DirInOmigaAlphaBeta);
            NormVec  = DirInOmigaAlphaBeta/DirNorm;
            a = NormVec(1)*NormVec(1) + NormVec(2)*NormVec(2);
            b = 2*(NormVec(1)*SourcePos_Psi_ellipse(1)+NormVec(2)*SourcePos_Psi_ellipse(2));
            c = SourcePos_Psi_ellipse(1)*SourcePos_Psi_ellipse(1) + SourcePos_Psi_ellipse(2)*SourcePos_Psi_ellipse(2) - radius*radius;
            delta = b*b - 4*a*c;
            D_Vec_Matrix(i,j,:) = NormVec;
            
            
%             D_Vec_Matrix_ellipse(i,j,:)=NormVec_ellipse;
            ...
            if delta>0
                ...
                t1 = (-b-sqrt(delta))/(2*a);
                sam_firstpos = SourcePos_Psi_ellipse + t1*NormVec;
                ...
                for k=1:nLength
                       ...
                       if slicepos>=398 | hori_pos>=499 | vert_pos>=499
                           continue;
                             ...
                             inter_pos = fix(sam_pos)+[500,500,400]';
...
%     fclose(fileID);
    ...
    dicomwrite(I,[file,'ellipse_cylinderballinclineprojdata\',filename]);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end;
