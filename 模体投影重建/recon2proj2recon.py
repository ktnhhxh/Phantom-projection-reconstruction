...
# 重建图所在位置
# filePath = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球img/模体/img_dcm/'
filePath = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球模体/418-模体/img/'
# 投影图保存位置
# proj_path = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球img/模体/proj/'
# recon_path = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球img/模体/recon'
proj_path = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球模体/师姐/proj/'
recon_path = 'D:/学习/研究生/Code/师姐重建代码/老师投影重建代码/data/圆柱球模体/师姐/recon/'
# 文件读取
...
thetaX = 0/180*np.pi  # 我们的坐标系中倾角
thetaY = 0/180*np.pi
thetaZ = 0/180*np.pi
# Ru1 = math.sin(thetaZ)*math.cos(thetaX)-math.cos(thetaZ)*math.sin(thetaY)*math.sin(thetaX)
# Ru2 = -math.cos(thetaZ)*math.cos(thetaX)-math.sin(thetaZ)*math.sin(thetaY)*math.sin(thetaX)
# Ru3 = -math.cos(thetaY)*math.sin(thetaX)
#
# Rv1 = math.sin(thetaZ)*math.sin(thetaX)+math.cos(thetaZ)*math.sin(thetaY)*math.cos(thetaX)
# Rv2 = -math.cos(thetaZ)*math.sin(thetaX)+math.sin(thetaZ)*math.sin(thetaY)*math.cos(thetaX)
# Rv3 = math.cos(thetaY)*math.cos(thetaX)

Ru1 = math.cos(thetaZ)*math.cos(thetaY)-math.sin(thetaZ)*math.sin(thetaX)*math.sin(thetaY)
Ru2 = math.sin(thetaZ)*math.cos(thetaY)+math.cos(thetaZ)*math.sin(thetaX)*math.sin(thetaY)
Ru3 = -math.sin(thetaX)*math.cos(thetaY)

Rv1 = math.cos(thetaZ)*math.sin(thetaY)+math.sin(thetaZ)*math.sin(thetaX)*math.cos(thetaY)
Rv2 = math.sin(thetaZ)*math.sin(thetaY)-math.cos(thetaZ)*math.sin(thetaX)*math.cos(thetaY)
Rv3 = math.cos(thetaX)*math.cos(thetaY)

for i in range(360):
    # source
    vectors[i, 0] = math.sin(angles[i]) * c
    ...
    vectors[i, 6] = math.cos(angles[i])*Ru1 - math.sin(angles[i])*Ru2
    vectors[i, 7] = math.sin(angles[i])*Ru1 + math.cos(angles[i])*Ru2
    vectors[i, 8] = Ru3

    # vector from detector pixel (0, 0) to (0, 1)  v
    vectors[i, 9] = math.cos(angles[i])*Rv1 - math.sin(angles[i])*Rv2
    vectors[i, 10] = math.sin(angles[i])*Rv1 + math.cos(angles[i])*Rv2
    vectors[i, 11] = Rv3
